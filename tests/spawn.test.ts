...
describe('SpawnManager', () => {
  ...
  it('pops a ghost near where the player is looking after a steady dwell', () => {
    const mgr = makeManager(GAZE);
    expect(revealed(step(mgr, FRONT, 60))).toHaveLength(0); // dwell 60 < 100
    const ev = revealed(step(mgr, FRONT, 60)); // dwell 120 ≥ 100 → pop
  ...
  it('respects the gaze cooldown between pops', () => {
    const mgr = makeManager(GAZE);
    step(mgr, FRONT, 60);
    step(mgr, FRONT, 60); // first pop; cooldown 1000ms
  ...
  it('warns first, then fires a behind-the-player ambush after the lead time', () => {
    ...
    expect(warn.some((e) => e.type === 'warning')).toBe(true);
